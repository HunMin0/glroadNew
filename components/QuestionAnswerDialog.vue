<template>
  <v-dialog v-model="dialog" persistent max-width="600px">
    <template v-slot:activator="{ on, attrs }">
      <v-btn v-bind="attrs" v-on="on" class="replebtn" elevation="0" title="문의답변"><v-icon small>mdi-pencil</v-icon>&nbsp;문의답변</v-btn>
    </template>
    <v-card class="boxditale">
      <v-card-title class="text-h6 font-weight-bold font-color1"><v-icon color="6c6c6c">mdi-tooltip-text</v-icon>&nbsp; 문의답변</v-card-title>
      <v-card-text class="bottom-padding-0">
        <v-textarea :value="form.content" label="문의내용" outlined readonly></v-textarea>
        <v-textarea v-model="form.answer" :error-messages="errors.answer"
                    counter="1000" :rules="[rules.length(1000)]"
                    outlined label="관리자님, 답변 내용을 입력해주세요." placehoder="답변 내용을 입력하세요."></v-textarea>
      </v-card-text>
...
export default {
  ...
  methods: {
    ...
    async init() {
      const data = (await this.$axios.get('/forwarding/questions/'+this.id)).data;
      this.form = data;
    },
    save() {
      if (this.form.answered_at && !confirm("이전 답변내용은 삭제됩니다.")) return false;
      this.errors = {};
      this.$axios.put('/forwarding/questions/'+this.id, this.form).then(response => {
