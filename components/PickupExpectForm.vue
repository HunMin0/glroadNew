<template>
  <v-dialog v-model="dialog" width="500">
    <template v-slot:activator="{ on, attrs }">
      <v-btn color="primary" v-bind="attrs" v-on="on"> 픽업예정일시 </v-btn>
    </template>
    <v-card>
      <v-card-title> 픽업예정일시 </v-card-title>
...
export default {
  ...
  watch: {
    dialog(value) {
      ...
      if (value) {
        //console.log(this.contract.pickup_expected_at, this.$dayjs(this.contract.pickup_expected_at).tz('Asia/Seoul'));
        const pickup_expected_at = this.$dayjs(this.contract.pickup_expected_at).tz('Asia/Seoul');
        this.form.pickup_expected_date = pickup_expected_at.format('YYYY-MM-DD');
        this.form.pickup_expected_time = pickup_expected_at.format('HH:mm');
      }
    }
  },
  data: () => ({
  ...
  methods: {
    ...
    async save() {
      try {
        this.errors = {};
        const data = await this.$axios.$put('/forwarding/contracts/'+this.contract.id+'/pickup-expect', this.form)
        if (data.result) {
          this.showSnackbar({show: true, color: 'success', message: '픽업예정일시가 등록 되었습니다.'});
          this.dialog = false;
          this.$emit('update');
        } else {
