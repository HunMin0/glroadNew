              ...
              <v-list-item three-line>
                <v-list-item-content>
                  <v-list-item-title class="text-h5 mb-0">
                    <v-icon class="iconMa">mdi-content-duplicate</v-icon> 사용자리뷰 내역
                  </v-list-item-title>
                  <v-list-item-subtitle>사용자 리뷰 상태내역을 확인 해주세요.</v-list-item-subtitle>
                </v-list-item-content>
          ...
          <v-row>
            <v-col class="top-padding-0">
                ...
                <template v-slot:item.rating="{ item }">
                  <v-rating :value="item.rating" readonly color="yellow darken-3"
                            dense half-increments background-color="grey darken-1" empty-icon="$ratingFull"/>
                </template>
                <template v-slot:item.actions="{ item }">
                <review-answer-dialog :id="item.id" @update="searchItem"></review-answer-dialog>
              </template>
              </v-data-table>
            </v-col>
          </v-row>
...
import ReviewAnswerDialog from "../../../components/ReviewAnswerDialog";

export default {
  name: "index",
  layout: "forwarding",
  components: {ReviewAnswerDialog, Search},
  watch: {
    pagination: { handler() { this.searchItem(); }, deep: true },
  },
  data: () => ({
    headers: [
      {align: 'left', sortable: false, width: '50%', text: '사용자 리뷰', value: 'content'},
      {align: 'center', sortable: false, width: '10%', text: '별점', value: 'rating'},
      {align: 'center', sortable: false, width: '20%', text: '등록일시', value: 'created_at'},
      {align: 'center', sortable: false, width: '10%', text: '상태', value: 'status_name'},
